...
function formatDate(dateString) {
    let parts = dateString.split("-"); // Tách theo dấu -
    return parts.reverse().join("/");  // Đảo ngược và nối lại bằng /
}

function saveChanges() {
    let tenMoi = document.getElementById("edit_ten").value;
    let ngaySinhMoi = document.getElementById("edit_ngaysinh").value;
    let sdtMoi = document.getElementById("edit_sodienthoai").value;

    // Chuyển đổi định dạng ngày
    let ngaySinhFormatted = formatDate(ngaySinhMoi);

    // Lấy tất cả thẻ <b> trong class hop1
    let hop1 = document.querySelector(".hop1").getElementsByTagName("b");

    // Cập nhật thông tin
    if (hop1.length >= 3) {
        hop1[0].innerText = tenMoi;           // Họ và tên
        hop1[1].innerText = ngaySinhFormatted; // Ngày sinh (định dạng DD/MM/YYYY)
        hop1[2].innerText = sdtMoi;           // Số điện thoại
    }

    hideEditForm();
}
document.getElementById("edit_ten").addEventListener("keypress", function (event) {
    let char = String.fromCharCode(event.which); 
    if (/\d/.test(char)) {
        event.preventDefault(); // Chặn nhập số
    }
});

document.getElementById("edit_sodienthoai").addEventListener("keypress", function (event) {
    let char = String.fromCharCode(event.which);
    if (!/\d/.test(char)) {
        event.preventDefault(); // Chỉ cho nhập số
    }
});
//////////////////////////////////////////////////////////////////
//Hiện Bảng Giao Diện
